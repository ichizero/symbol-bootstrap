...
export class RunService {
                    ...
                    (service.ports || []).map(async (portBind) => {
                        ...
                        if (service.container_name.indexOf('node-agent') > -1) {
                            ...
                            try {
                                ...
                                this.logger.info(`Agent ${testUrl} is up and running...`);
                                return true;
                            } catch (e) {
                                this.logger.warn(`Agent ${testUrl} is NOT up and running YET: ${e.message}`);
                                return false;
                            }
                        }
                        return true;
    ...
    private async basicRun(extraArgs: string[]): Promise<string> {
        ...
        const dockerComposeArgs = ['-f', dockerFile];
        const args = [...dockerComposeArgs, ...extraArgs];
        return BootstrapUtils.spawn('docker-compose', args, this.logger, false);
    }
